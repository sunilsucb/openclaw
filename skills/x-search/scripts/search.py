...
TIMEOUT_S = 90
MAX_RESPONSE_BYTES = 2 * 1024 * 1024  # 2MB default
MAX_RETRIES = 3
MAX_HANDLES = 10
DATE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")
HANDLE_RE = re.compile(r"^[a-zA-Z0-9_]{1,15}$")
FLAGS_WITH_VALUES = {"--handles", "--exclude", "--from", "--to", "--model", "--max-response"}
...
def parse_args(argv: list[str]) -> dict:
    ...
    if not args or args[0] in ("-h", "--help"):
        print(
            'Usage: python3 search.py [--handles h1,h2] [--exclude h1,h2] [--from YYYY-MM-DD] [--to YYYY-MM-DD] [--images] [--video] [--model MODEL] [--max-response BYTES] "<query>"',
            file=sys.stderr,
        )
    ...
    while i < len(expanded):
        ...
        elif not flags_done and arg == "--model":
            i += 1
            options["model"] = expanded[i]
        elif not flags_done and arg == "--max-response":
            i += 1
            try:
                options["max_response"] = int(expanded[i])
            except ValueError:
                die(f"--max-response must be an integer, got \"{expanded[i]}\".")
...
def validate(options: dict) -> None:
    ...
    if options["max_response"] <= 0:
        die("--max-response must be a positive integer.")
...
def _read_limited(resp, max_bytes: int) -> bytes:
    """Read response with a size limit to prevent memory exhaustion."""
    chunks = []
    total = 0
    while True:
        ...
        total += len(chunk)
        if total > max_bytes:
            die(f"Response exceeded max size ({max_bytes} bytes). Use --max-response to increase.")
        chunks.append(chunk)
...
def search(options: dict) -> None:
    ...
    max_response = options.get("max_response", MAX_RESPONSE_BYTES)

    for attempt in range(MAX_RETRIES + 1):
        try:
            with opener.open(req, timeout=TIMEOUT_S) as resp:
                raw = _read_limited(resp, max_response)
                data = json.loads(raw)
            break
